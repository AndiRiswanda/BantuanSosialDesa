...
function ProgramCard({ item }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 sm:p-5 space-y-4">
      {/* Received badge (conditional) */}
      {item.received && (
        <div className="flex items-center gap-2 text-emerald-700 text-xs sm:text-sm font-semibold rounded-md border border-emerald-200 bg-emerald-50 px-3 py-1.5">
          <BadgeCheck className="w-4 h-4" />
          Anda menerima bantuan ini
        </div>
      )}
      {/* Header */}
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
        ...
        <p className="mt-1 text-sm text-slate-700">
          {item.type === "Uang" ? item.amount : item.goods}
        </p>
      </div>
      ...
      <div className="flex justify-center">
        <button
          onClick={() => navigate(`/penerima/program/${item.id}`)}
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-20 py-6 lg:py-10">
        ...
        <div className="text-center mb-6">
          <div className="inline-block bg-white shadow-md border border-blue-200 px-6 sm:px-8 py-3 rounded-lg">
            <h1 className="text-[#0B2B5E] font-semibold text-lg sm:text-xl">Program Bantuan Desa</h1>
            <p className="text-xs text-slate-600 mt-1">Jelajahi, pantau, dan evaluasi berbagai program bantuan sosial yang tersedia di Desa Anda.</p>
          </div>
        </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(["Semua", "Terjadwal", "Selesai"]).map((s) => (
                  <StatusPill key={s} active={statusFilter === s} onClick={() => setStatusFilter(s)}>{s}</StatusPill>
                ))}
              </div>
            </div>
            {/* Type group */}
