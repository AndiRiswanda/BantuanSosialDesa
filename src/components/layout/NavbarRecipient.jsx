...
export default function NavbarRecipient() {
  const { pathname } = useLocation();
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);
  const items = [
    { to: "/penerima", label: "Beranda" },
    { to: "/penerima/program", label: "Program Bantuan" },
    { to: "/penerima/pengajuan", label: "Pengajuan Bantuan" },
  ];
  ...
  return (
    <nav className="bg-[#1976d26c] relative flex justify-between items-center px-6 py-3 shadow-sm">
      ...
      <ul className="hidden md:flex space-x-8 text-[#0B2B5E] font-medium">
        {items.map((item) => {
          const active = item.to === "/penerima" ? pathname === item.to : pathname.startsWith(item.to);
          return (
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => navigate("/penerima/profil")} className="text-[#0B2B5E] hover:text-green-700">
