    ...
    <nav className="bg-[#1976d26c] relative flex justify-between items-center px-6 py-3 shadow-sm">
      ...
      <ul className="hidden md:flex space-x-8 text-[#0B2B5E] font-medium">
        {items.map((item) => (
          <li
            key={item.to}
            className={`${(item.to === "/donor" ? pathname === item.to : pathname.startsWith(item.to)) ? "border-b-4 border-[#43A047]" : "hover:text-green-600"} pb-1`}
          >
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => navigate("/donor/profil")} className="text-[#0B2B5E] hover:text-green-700">
      ...
      {open && (
        ...
        >
          <ul className="flex flex-col py-2 text-[#0B2B5E] font-medium">
            {items.map((item) => {
              const active = item.to === "/donor" ? pathname === item.to : pathname.startsWith(item.to);
              return (
                <li key={item.to}>
                  <Link
                    to={item.to}
