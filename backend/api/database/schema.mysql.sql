...
CREATE TABLE `penerima` (
  ...
  `pekerjaan_istri` VARCHAR(100) NULL DEFAULT NULL,
  `status_anak` VARCHAR(100) NULL DEFAULT NULL,
  `jumlah_tanggungan` INT NULL DEFAULT 0,
  `penghasilan` ENUM('< Rp 500.000', 'Rp 500.000 - Rp 1.000.000', 'Rp 1.000.000 - Rp 2.000.000', 'Rp 2.000.000 - Rp 3.000.000', '> Rp 3.000.000') NULL,
...
CREATE TABLE `program_bantuan` (
  ...
  `status` ENUM('pending', 'aktif', 'selesai', 'ditunda') NOT NULL DEFAULT 'pending',
...
CREATE TABLE `transaksi_penyaluran` (
  ...
  `status_penyaluran` ENUM('dijadwalkan', 'selesai', 'dibatalkan') NOT NULL,
